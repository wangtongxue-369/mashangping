...
describe('CourseAssignmentsPage', () => {
  ...
  it('渲染作业行：标题/状态徽标（未开始、进行中），并提供返回入口', async () => {
    mocks.get.mockImplementation((url: string) => {
      if (url === '/courses/my') return ok([{ courseId: 7, name: '数据结构', term: '2026-2027-1' }]);
      return ok(assignmentsPage);
    });
    renderPage();
    ...
    // 页眉带课程名上下文（深链也有课程名）
    expect(screen.getByText('数据结构 · 作业列表')).toBeTruthy();
  ...
  it('作业列表接口失败显示错误卡 + 重试（不伪装空数据）', async () => {
    mocks.get.mockRejectedValue({ message: '服务暂不可用', code: 500 });
    renderPage();
    expect(await screen.findByText('作业列表加载失败，请稍后重试')).toBeTruthy();
    expect(screen.getByRole('button', { name: /重\s*试/ })).toBeTruthy();
    expect(screen.queryByText('未开始作业')).toBeNull();
  });
});
